...
class EnhancedAdminDashboard extends Page
{
    ...
    public function getUserManagementSummary(): array
    {
        return Cache::remember('admin_user_management', now()->addMinutes(15), function () {
            ...
            $staffCount = ($usersByRole['petugas'] ?? 0) + ($usersByRole['paramedis'] ?? 0);
            $doctorCount = ($usersByRole['dokter'] ?? 0) + ($usersByRole['dokter_gigi'] ?? 0);
    ...
    public function getSystemPerformance(): array
    {
        return Cache::remember('admin_system_performance', now()->addMinutes(5), function () {
            ...
            $avgResponseTime = $metrics->avg('response_time') ?? 0;
            
            // Calculate database queries per second (estimated)
            $dbQueriesPerSec = $databaseQueries > 0 ? round($databaseQueries / 60, 1) : 0; // Assuming metrics are per minute
            
            // Calculate uptime percentage (based on successful metrics collection)
            $uptimePercentage = $metrics->count() > 0 ? min(99.9, round((1 - ($queueJobs / 1000)) * 100, 1)) : 99.9;
            ...
            return [
                ...
                'db_queries_per_sec' => $dbQueriesPerSec,
    ...
    public function getFinancialOverview(): array
    {
        return Cache::remember('admin_financial_overview', now()->addMinutes(30), function () {
            ...
            $dailyRevenue = PendapatanHarian::whereDate('tanggal_input', now()->toDateString())
                ->sum('nominal');
            
            $dailyExpenses = PengeluaranHarian::whereDate('tanggal_input', now()->toDateString())
                ->sum('nominal');
            ...
            return [
                ...
                // Monthly data (with aliases for view compatibility)
                'current_revenue' => $currentRevenue,
                'monthly_revenue' => $currentRevenue,
                'current_expenses' => $currentExpenses,
                'monthly_expenses' => $currentExpenses,
                
                // Net calculations (with aliases for view compatibility)
    ...
    public function getMedicalOperations(): array
    {
        return Cache::remember('admin_medical_operations', now()->addMinutes(20), function () {
            ...
            // Today's data
            $dailyPatients = JumlahPasienHarian::whereDate('tanggal', $today)
                ->sum(DB::raw('jumlah_pasien_umum + jumlah_pasien_bpjs'));
            
            $dailyProcedures = Tindakan::whereDate('tanggal_tindakan', $today)->count();
            
            // Patient statistics (monthly)
            $totalPatients = JumlahPasienHarian::whereMonth('tanggal', $currentMonth->month)
                ->whereYear('tanggal', $currentMonth->year)
                ->sum(DB::raw('jumlah_pasien_umum + jumlah_pasien_bpjs'));
            
            // Procedures completed (monthly)
            $proceduresCompleted = Tindakan::whereMonth('tanggal_tindakan', $currentMonth->month)
                ->whereYear('tanggal_tindakan', $currentMonth->year)
                ->count();
            
            // Emergency cases (estimated from today's procedures - assuming 20% are emergency)
            $emergencyCases = round($dailyProcedures * 0.2);
            
            // Routine checkups (estimated - 50% of procedures)
            $routineCheckups = round($dailyProcedures * 0.5);
            
            // Follow-ups (estimated - 30% of procedures)
            $followUps = round($dailyProcedures * 0.3);
            ...
            return [
                // Daily data for dashboard display
                'daily_patients' => $dailyPatients,
                'daily_procedures' => $dailyProcedures,
                'emergency_cases' => $emergencyCases,
                'routine_checkups' => $routineCheckups,
                'follow_ups' => $followUps,
                
                // Monthly data (keep existing keys for compatibility)
    ...
    public function getSixMonthTrends(): array
    {
        return Cache::remember('admin_six_month_trends', now()->addMinutes(60), function () {
            ...
            // Ensure no division by zero in view by adding minimum values
            $maxRevenue = max($revenue);
            $maxPatients = max($patients);
            
            // If all values are zero, add a minimum value to prevent division by zero
            if ($maxRevenue === 0) {
                $revenue = array_map(function($value) { return max($value, 1); }, $revenue);
            }
            
            if ($maxPatients === 0) {
                $patients = array_map(function($value) { return max($value, 1); }, $patients);
            }
