...
class AdminInteractiveDashboardWidget extends Widget implements HasForms
{
                ...
                Section::make('🏥 Kontrol Dashboard Medis')
                    ...
                    ->schema([
                        Select::make('selectedPeriod')
                            ...
                            ->live()
                            ->afterStateUpdated(fn () => $this->debounceRefreshData()), // Debounced to prevent loops
    ...
    protected function debounceRefreshData(): void
    {
        // Debounce refresh to prevent excessive Livewire updates
        static $lastRefresh = 0;
        $now = time();
        
        if ($now - $lastRefresh >= 3) { // 3 second debounce
            $lastRefresh = $now;
            $this->refreshData();
        }
    }
