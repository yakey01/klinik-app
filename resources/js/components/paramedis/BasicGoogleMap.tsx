...
export function BasicGoogleMap({ onLocationSelect, height = '300px' }: BasicGoogleMapProps) {
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      ...
      try {
        ...
        const mapOptions: google.maps.MapOptions = {
          center: { lat: -7.808758, lng: 111.962646 }, // Use coordinates from screenshot
          zoom: 16,
    ...
    const loadGoogleMaps = () => {
      ...
      // Use a test key or load without key for basic functionality
      const apiKey = import.meta.env.VITE_GOOGLE_MAPS_API_KEY;
      if (apiKey && apiKey !== 'AIzaSyD_your_api_key_here') {
        script.src = `https://maps.googleapis.com/maps/api/js?key=${apiKey}&libraries=places`;
      } else {
        console.warn('🚨 No valid Google Maps API key found, loading basic maps...');
